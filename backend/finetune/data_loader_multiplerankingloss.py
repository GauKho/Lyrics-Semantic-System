...
def create_training_pairs_augmented(df, negative_ratio=2.0):
    train_examples = []
    all_titles = df["Title"].dropna().tolist()
    all_albums = df["Album"].dropna().tolist()
    # all_artists = df["Artist"].tolist()

    for _, row in df.iterrows():
        if  pd.isna(row["Lyric"]) or pd.isna(row["Title"]) or pd.isna(row["Album"]):
            continue
        lyric = _normalize_text(row["Lyric"])
        title = _normalize_text(row["Title"])
        album = _normalize_text(row["Album"])
        # artist = map(_normalize_text, row["Artist"])

         # Bỏ qua nếu lyric rỗng
        if not lyric:
            continue
        ...
        if len(lyric.split()) > 5:
            ...
            train_examples.append(InputExample(texts=[lyric, lyric]))

            # Hard negative pairs
            for _ in range(int(negative_ratio)):
                wrong_title = random.choice(all_titles)
                neg_album = random.choice(all_albums)
                # neg_artist = random.choice(all_artists)
                if neg_album != album:
                    train_examples.append(InputExample(texts=[lyric, neg_album]))
